...
class TestOnExactitudeInScience <  MiniTest::Test
  ...
  def test_it_will_never_actually_run_because_of_math
    end_of_the_universe = Time.now + (17_000_000_000 * 365 * 24 * 60 * 60)
    assert(Time.now + (CHARACTERS.length ** BABELTEXT.length)/1_000_000 > end_of_the_universe)
  end
